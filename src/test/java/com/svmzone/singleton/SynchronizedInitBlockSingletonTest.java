...
@SpringBootTest
public class SynchronizedInitBlockSingletonTest {

    @Test
    void SynchronizedInitBlockSingleton() throws InterruptedException {
        ...
        for (int i = 0; i < Const.NUMBER_OF_THREADS; i++) {
            tasks.add(() -> SynchronizedInitBlockSingleton.getInstance());
        }
        long execTime = System.currentTimeMillis();
        executor.invokeAll(tasks);
        System.out.println("Execution time: " + (System.currentTimeMillis() - execTime));
        assertEquals(1, SynchronizedInitBlockSingleton.instanceCount.get(), "More than one instance was created");
    }
}
